...
function Categories() {
  ...
  const getOrders = async () => {
    try {
        setLoading(true)
        const response = await axios.get(`https://airobike.unotaxi.xyz/api/v1/categories/list/`)
        .then(res => {
    ...
    (o)=>{
        return(
            <tr>
                ...
                <td style={{paddingLeft: '10px', paddingRight: '10px', fontSize: '13px'}}>{o.ordering}</td>
                <td style={{paddingLeft: '10px', paddingRight: '10px', fontSize: '13px'}}>{o.pCategory?.length}</td>
