...
function SubCategories() {
  ...
  const getOrders = async () => {
    try {
        setLoading(true)
        const response = await axios.get(`https://airobike.unotaxi.xyz/api/v1/subcategories/list/`)
        .then(res => {
    ...
    (o)=>{
        return(
            <tr>
                ...
                <td style={{paddingLeft: '10px', paddingRight: '10px'}}><p style={{fontSize: '13px'}}>{o.ordering}</p></td>
                <td style={{paddingLeft: '10px', paddingRight: '10px'}}><p style={{fontSize: '13px'}}>{o.category?.title}</p></td>
                <td style={{paddingLeft: '10px', paddingRight: '10px'}}><p style={{fontSize: '13px'}}>{o.pSubCategory?.length}</p></td>

                <td style={{paddingLeft: '20px'}}>
                  <MDTypography component="a" href={'subcategories/' + o._id} variant="caption" style={{color: 'blue'}} fontWeight="medium">
                    Редактировать
                  </MDTypography>
                </td>
            </tr>
                ...
                <div style={{paddingLeft: '10px', paddingRight: '10px'}}>
                  <table class="table table-striped">
                      <thead>
                          ...
                          <th style={{fontSize: '13px', paddingLeft: '10px', paddingRight: '10px'}}>Категория</th>
